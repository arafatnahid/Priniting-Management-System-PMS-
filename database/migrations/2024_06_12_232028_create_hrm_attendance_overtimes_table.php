...
return new class extends Migration
{
    ...
    {
        Schema::create('hrm_attendance_overtimes', function (Blueprint $table) {
            $table->id();
			//
			$table->foreignId('employee_id')->constrained('users')->onUpdate('cascade')->onDelete('cascade');
			$table->foreignId('attendance_id')->constrained('hrm_attendances')->onUpdate('cascade')->onDelete('cascade');
			$table->date('date');
    ...
    {
        Schema::dropIfExists('hrm_attendance_overtimes');
    }
};
